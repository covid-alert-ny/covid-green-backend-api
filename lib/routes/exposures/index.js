...
const { Forbidden, Gone, InternalServerError, TooManyRequests } = require('http-errors')
const {
  differenceInMinutes,
  differenceInSeconds,
  isAfter
...
async function exposures(server, options, done) {
  ...
  server.route({
    ...
    handler: async request => {
      ...
      let registerRateLimit
      try {
        registerRateLimit = (await server.pg.write.query(
          registerUpdate({
        ...
        if (isProduction) {
          throw new InternalServerError('An error occurred while verifying the exposure.')
        } else {
          throw new InternalServerError('An error occurred while verifying the exposure'
            + ' and updating the registrations.last_verification_attempt:\n',
      ...
      let controlRows
      try {
        controlRows = (await server.pg.write.query(
          verificationUpdate({
            rateLimit: options.security.verifyRateLimit,
            control
          })
        ...
        if (isProduction) {
          ...
          throw new InternalServerError('An error occurred while verifying the exposure'
            + ' and updating the verifications.last_attempt:\n',
            + `query: ${JSON.stringify({
              rateLimit: options.security.verifyRateLimit,
              control
            }, null, 2)}\n`
      ...
      let hashRows;
      try {
        hashRows = (await server.pg.write.query(
          verificationDelete({
            control,
            code
          })
        ...
        if (isProduction) {
          ...
          throw new InternalServerError('An error occurred while verifying the exposure'
            + ' and deleting the verifications record:\n',
            + `query: ${JSON.stringify({
              control,
              code
            }, null, 2)}\n`
      ...
      try {
        ...
        if (isProduction) {
          ...
          throw new InternalServerError('An error occurred while verifying the exposure'
            + ' and inserting into upload_tokens:\n',
            + `query: ${JSON.stringify({ id, onsetDate }, null, 2)}\n`
            + err)
        }
      }
    }
  ...
  server.route({
    ...
    handler: async (request, response) => {
      ...
      let rows;
      try {
        rows = (await server.pg.write.query(tokenDelete({ id, token }))).rows
      } catch (err) {
        if (isProduction) {
          throw new InternalServerError('An error occurred while updating exposures.');
        } else {
          throw new InternalServerError('An error occurred while updating exposures'
            + ' and deleting from upload_tokens:\n',
            + `query: ${JSON.stringify({ id, token }, null, 2)}\n`
            + err)
        }
      }
      ...
      try {
        ...
        // TODO: Do we want to throw an error here in production???
        if (isProduction) {
          throw new InternalServerError('An error occurred while updating exposures.');
        } else {
          throw new InternalServerError('An error occurred while updating exposures'
            + ' and inserting into metrics:\n',
            + `query: ${JSON.stringify({
              event: 'UPLOAD',
              os: platform,
              version: ''
  ...
  server.route({
    ...
    handler: async request => {
      ...
      try {
        ...
        // TODO: Do we want to throw an error here in production???
        if (isProduction) {
          throw new InternalServerError('An error occurred while getting exposures.');
        } else {
          throw new InternalServerError('An error occurred while getting exposures'
