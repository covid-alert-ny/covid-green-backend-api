...
const { NotFound, InternalServerError, Unauthorized } = require('http-errors')
...
async function register(server, options, done) {
  ...
  server.route({
    ...
    handler: async () => {
      ...
      try {
        ...
        if (isProduction) {
          throw new InternalServerError('An error occurred while creating registration.')
        } else {
          throw new InternalServerError('An error occurred while creating registration'
            + ' and inserting into registrations:\n',
            + `query: ${JSON.stringify({ nonce }, null, 2)}\n`
            + err)
        }
      }
  ...
  server.route({
    ...
    handler: async request => {
      ...
      let rowCount, rows
      try {
        const result = await server.pg.read.query(
        ...
        if (isProduction) {
          throw new InternalServerError('An error occurred while updating registration.')
        } else {
          throw new InternalServerError('An error occurred while updating registration'
            + ' and selecting from registrations:\n',
            + `query: ${JSON.stringify({ nonce }, null, 2)}\n`
            + err)
        }
      }
