  ...
  if (options.routes.metrics) {
    server.route({
      ...
      handler: async (request, response) => {
        ...
        let rows;
        try {
          rows = (await server.pg.write.query(
            requestInsert({
              event,
              id,
          ...
          if (isProduction) {
            throw new InternalServerError('An error occurred while inserting metric.')
          } else {
            throw new InternalServerError('An error occurred while inserting metric'
              + ' and inserting into metric_requests:\n',
        ...
        try {
          ...
          if (isProduction) {
            ...
            throw new InternalServerError('An error occurred while inserting metric'
              ...
              + `query: ${JSON.stringify({ event, os, version }, null, 2)}\n`
              + err)
          }
        }
