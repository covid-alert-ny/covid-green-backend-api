  ...
  if (options.routes.callback) {
    ...
    server.route({
      ...
      handler: async (request, response) => {
        ...
        let rowCount;
        try {
          rowCount = (await server.pg.write.query(registerUpdate({ id }))).rowCount
        } catch (err) {
          if (isProduction) {
            throw new InternalServerError('An error occurred while requesting callback.')
          } else {
            throw new InternalServerError('An error occurred while requesting callback'
              + ` and updating registration (id = ${id}): `
              + err)
          }
        }
