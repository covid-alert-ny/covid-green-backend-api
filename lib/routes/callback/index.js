  ...
  if (options.routes.callback) {
    ...
    server.route({
      ...
      handler: async (request, response) => {
        ...
        const query =
          options.security.callbackRateLimitSeconds === undefined ||
          options.security.callbackRateLimitSeconds === null ||
          options.security.callbackRateLimitSeconds < 0
            ? registerOnetimeCallback({ registrationId: id })
            : registerRateLimitedCallback({
                registrationId: id,
                rateLimitSeconds: options.security.callbackRateLimitSeconds
              })
